...
/* #define PRINTOUT */
	  ...
	  printf("Received %d triggers... \n",
		 event_number);
#endif
/* 	  if( abs(len-prev_len)>4 ) */
/* 	    printf("%8d: unusual event.  length = %d\n",event_number,len); */
	  for(ii=0;ii<len;ii++) 
	    {
	  ...
	  if(shmEvent)
	    {
/* 	      crateShmSetData(event_number, shmData, len); */
	    }
#ifdef PRINTOUT
	  printf("\n\n");
#endif
...
/*       if(clocksource!=2) */
/* 	{ */
/* 	  clocksource = tiGetClockSource(1); */
/* 	  printf("%s: ERROR.  TI CLOCK SOURCE is NOT FIBER. (%s)", */
/* 		 __FUNCTION__,clocksource); */
